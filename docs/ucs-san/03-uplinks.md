...
This lab treats the LAN uplink from each FI as a **port channel**, not a single link, from the start — that's the design every later module (Task 2.4's VLAN Groups, Module 7's trunk verification, Module 9's load-balancing hash) already assumes. N5K-1 and N5K-2 run a **vPC domain**, so each FI's two physical uplinks — one to N5K-1, one to N5K-2 — bundle into a single logical port channel that both N5K peers treat as one link.
...
**On N5K-1/N5K-2**, first create the local VLANs in the switch's own VLAN database (this is separate from the UCSM VLAN objects you created in Module 1, Task 1.1 — matching IDs are what tie the two together, not a shared database). vPC needs a peer-keepalive link (routed, typically over the mgmt0 VRF) confirmed up *before* the peer-link/domain will pass traffic, and a `vpc domain` shared by both switches. VLAN 10 and 11 are shared, so both port channels — the one toward FI-A and the one toward FI-B — carry the same Data/Mgmt pair; the allowed-VLAN list belongs on the **port-channel interface**, not repeated on each physical member. The one thing that must *not* be shared is the FCoE VLAN: it's bound to that fabric's VSANs (Module 4), so mixing FCoE VLAN 1000 onto the FI-B port channel (or 2000 onto the FI-A one) would leak one fabric's SAN traffic onto the other's Ethernet path and defeat the point of keeping the SAN fabrics isolated:

??? "Commands"
    ```text
    ! On both N5K-1 and N5K-2 — identical vpc domain/peer-keepalive config on each
    feature vpc
    vpc domain 1
...
**Associate VLANs to the port channel.** Every VLAN defined under **LAN > LAN Cloud > VLANs** (Module 1, Task 1.1) is trunked on *every* uplink port and port channel by default — you don't have to do anything for VLAN 10/11 to ride the new port channel. To explicitly confirm (or scope) that association, open **LAN > LAN Cloud > VLANs > [VLAN 10 or 11] > Ports** (or use **LAN Uplinks Manager**) and add the Fabric A/B port channel there. Be deliberate about this: explicitly associating a VLAN to specific uplinks **prunes it off every uplink not listed** — since Task 2.4 already needs both VLANs reachable on both port channels, don't use this VLAN-to-port association to restrict anything here; use the VLAN Group *pinning* in Task 2.4 instead when you want a preferred-but-not-exclusive path.
...
Confirm both member links show `(P)` under the port channel, `show vpc` reports the vPC as `up` with consistent VLANs on both peers, and each port channel's "Vlans Allowed on Trunk" includes 10, 11, and its fabric's FCoE VLAN only. In UCSM, confirm the port channel's member ports show **Up**, and that the VLAN's **Ports** tab lists the port channel if you explicitly associated it.
...
3. **Match the N5K side:** the trunk `allowed vlan` list on both N5K port channels (Task 2.1) should already permit VLAN 10 and 11 on both — a VLAN Group pin doesn't need a matching restriction on the N5K allowed-list the way a hard-segregation design would, since both VLANs are legitimately allowed on both port channels:

??? "Commands"
    ```text
    ! N5K-1/N5K-2 — both VLANs are legitimately allowed on both port channels;
...
**Verify:** **LAN > LAN Cloud > VLAN Groups**, confirm each group's member VLAN and its pinned uplink; cross-check with `show interface trunk` on N5K-1/N5K-2 that VLAN 10 and 11 both appear on both port channels — that's expected here, unlike a hard-segregated design.
