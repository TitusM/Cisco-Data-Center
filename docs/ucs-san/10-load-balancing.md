...
Confirm this is actually happening rather than assumed: on N5K-1, `show interface port-channel1 counters` (or `show interface counters trunk`) should show VLAN 10's traffic dominating its pinned port channel under normal conditions, with VLAN 11 doing the same on its own pinned port channel — not a hard zero on the non-preferred side, since both VLANs remain permitted on both trunks.
